...
setwd("E:\\Toshiba_Desktop\\Stony Brook\\Teaching courses\\Chordate Spring 2016\\HeadTA_grading\\Lab")
cq_prac <- read.csv("CQ_Prac_comparison2.csv")
...
m.cq_prac2 <- melt(cq_prac2, id.vars= "Section",measure.vars = c("CQ1","TA1","CQ2", "TA2","CQ3", "TA3"), na.rm=T)
...
cqPlot <- ggplot(m.cq, aes(variable, as.numeric(as.character(value)))) + geom_boxplot(aes(fill = factor(Section))) +  
...
pracPlot <- ggplot(m.ta, aes(variable, as.numeric(as.character(value)))) + geom_boxplot(aes(fill = factor(Section))) +  
  geom_point() + facet_wrap(~Section, nrow=1) + theme_bw() + theme(legend.position = "none") +
...
cqPlot3 <- ggplot(m.cq3, aes(variable, as.numeric(as.character(value)))) + geom_boxplot(aes(fill = factor(Section))) +  
  geom_point() + facet_wrap(~Section, nrow=1) + theme_bw() + theme(legend.position = "none") + 
...
pracPlot3 <- ggplot(m.ta3, aes(variable, as.numeric(as.character(value)))) + geom_boxplot(aes(fill = factor(Section))) +  
  geom_point() + facet_wrap(~Section, nrow=1) + theme_bw() + theme(legend.position = "none") +
